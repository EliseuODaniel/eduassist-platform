...
_LANGUAGE_PREFERENCE_PATTERNS: tuple[re.Pattern[str], ...] = (
    re.compile(r"\b(?:fale|fala|responda|responde|escreva|escreve)\b.*\b(?:portugues|português|ingles|inglês|english|espanhol|spanish)\b"),
    re.compile(r"\bquero que\b.*\b(?:fale|fala|responda|responde)\b.*\b(?:portugues|português|ingles|inglês|english|espanhol|spanish)\b"),
    re.compile(r"\b(?:prefiro|apenas|so|só)\b.*\b(?:portugues|português|ingles|inglês|english|espanhol|spanish)\b"),
    re.compile(r"\b(?:ta|tá|esta|está|ficou|veio)\b.*\bem\b.*\b(?:ingles|inglês|english|portugues|português)\b"),
    re.compile(r"\bpor que\b.*\b(?:admissions|termo|palavra|mensagem|resposta|texto)\b.*\b(?:ingles|inglês|english|portugues|português)\b"),
    re.compile(r"\b(?:admissions|termo|palavra|mensagem|resposta|texto)\b.*\bem\b.*\b(?:ingles|inglês|english|portugues|português)\b"),
)
...
def normalize_ingress_text(text: str | None) -> str:
    normalized = unicodedata.normalize("NFKC", str(text or "")).lower().replace("º", "o").replace("ª", "a")
    chars: list[str] = []
...
def looks_like_language_preference_feedback(message: str | None) -> bool:
    ...
    if any(pattern.search(normalized) for pattern in _LANGUAGE_PREFERENCE_PATTERNS):
        return True
    has_meta_anchor = any(_contains_term(normalized, term) for term in _LANGUAGE_PREFERENCE_META_TERMS)
    has_academic_hint = any(_contains_term(normalized, term) for term in _LANGUAGE_SUBJECT_NEGATIVE_HINTS)
...
def looks_like_opaque_short_input(message: str | None) -> bool:
    ...
    if lowered in _OPAQUE_SHORT_INPUT_EXACT_EXCLUSIONS:
        return False
    if any(lowered == prefix or lowered.startswith(f"{prefix} ") for prefix in _OPAQUE_SHORT_INPUT_PREFIX_EXCLUSIONS):
        return False
...
def should_run_semantic_ingress_classifier(
    *,
    ...
    if str(current_domain or "").strip().lower() not in {"institution", "unknown"} and not (
        ...
        return False
    if str(current_mode or "").strip().lower() not in {"structured_tool", "clarify", "hybrid_retrieval"}:
        return False
    ...
    if looks_like_school_scope_message(message) and not (language_feedback or opaque_short_input):
        return False
    if normalized and any(_contains_term(normalized, term) for term in _STRONG_CONTENT_TERMS) and not (
        language_feedback or opaque_short_input or scope_boundary_candidate
    ):
        return False
...
def _validated_conversation_act(*, request_message: str, act: str) -> IngressAct:
    ...
    if looks_like_scope_boundary_candidate(request_message) and normalized_act in {"none", "input_clarification"}:
...
async def _text_call(
    *,
    ...
    if provider in {"google", "gemini", "litellm"} and getattr(settings, "google_api_key", None):
        ...
        endpoint = str(
            getattr(settings, "google_api_base_url", "https://generativelanguage.googleapis.com/v1beta")
        ).rstrip("/")
        payload = {
