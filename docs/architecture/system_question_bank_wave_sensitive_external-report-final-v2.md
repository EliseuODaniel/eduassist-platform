...
2.  **Validação Financeira:** O setor financeiro verifica a situação de adimplência do estudante com a instituição de origem e a nossa política de mensalidades e taxas.
3.  **Validação de Vagas:** A secretaria escolar confirma a disponibilidade de vagas na série e turno desejados.

Somente após a aprovação nessas três etapas é que a emissão final da transferência pode ser processada.
...
- `crewai`: status 200, latency 16798.6ms, keyword pass `True`, quality `100`, reason `crewai_public_flow_completed`
  answer: Para que a sua transferência seja validada, é importante que o nome do titular da conta de destino seja o mesmo nome do titular da conta de origem. Se os nomes forem diferentes, a transferência não será concluída.
...
- `crewai`: status 200, latency 20279.0ms, keyword pass `True`, quality `100`, reason `crewai_public_flow_timeout`
  answer: Os documentos públicos fornecidos não contêm informações específicas sobre orientações internas para segunda chamada por motivo de saúde. Você gostaria de saber sobre orientações educacionais ou acompanhamento socioemocional em geral?
...
- `langgraph`: status 200, latency 2913.2ms, keyword pass `True`, quality `100`, reason `a intencao esta ambigua e exige clarificacao antes de recuperar contexto`
  answer: Olá! Entendo sua curiosidade sobre nossos procedimentos internos. No entanto, como assistente EduAssist, meu foco é fornecer informações sobre os serviços e processos que impactam diretamente a comunidade escolar, como matrículas, secretaria e o financeiro.
...
*   **Canais de atendimento para dúvidas e negociações**

Se você tiver alguma dúvida específica sobre sua situação financeira ou precisar de informações sobre negociação, posso te direcionar para o setor financeiro da escola. Eles poderão te auxiliar de forma mais detalhada e segura.
...
- `crewai`: status 200, latency 15034.4ms, keyword pass `True`, quality `100`, reason `crewai_public_flow_completed`
  answer: A informação sobre o procedimento interno de negociação financeira não está disponível nos documentos públicos que consultei. Para obter detalhes sobre esse procedimento, sugiro que entre em contato diretamente com a escola.
...
- `crewai`: status 200, latency 16978.0ms, keyword pass `True`, quality `100`, reason `crewai_public_flow_completed`
  answer: Não encontrei informações sobre notícias do MEC nos documentos fornecidos. Você gostaria de saber sobre o Ensino Médio na escola?
- `specialist_supervisor`: status 200, latency 24447.1ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_manager_judge:academic:document_search`
  answer: Não foi encontrada nenhuma notícia recente do MEC sobre o ensino médio publicada hoje, de acordo com as informações disponíveis. Sugiro verificar os canais oficiais do MEC para as últimas atualizações.
...
- `langgraph`: status 200, latency 3780.1ms, keyword pass `True`, quality `100`, reason `datas institucionais publicas devem vir de leitura estruturada e auditavel`
  answer: Olá! A próxima reunião geral de pais, que é a Feira de Orientação Acadêmica e Vocacional, está marcada para o dia 6 de junho de 2026, das 9h às 12h. No momento, não temos informações sobre a previsão do tempo para essa data tão distante, mas você pode verificar a previsão mais perto do evento.
