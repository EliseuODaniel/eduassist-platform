...
Date: 2026-04-01T16:06:10.440656+00:00

Dataset: `/home/edann/projects/eduassist-platform/tests/evals/datasets/system_question_bank_wave_public_grounding.json`

Run prefix: `debug:five-path:20260401T160111Z`

## Stack Summary

| Stack | OK | Keyword pass | Quality | Avg latency |
| --- | --- | --- | --- | --- |
| `langgraph` | `24/24` | `24/24` | `100.0` | `2328.5 ms` |
| `crewai` | `24/24` | `24/24` | `100.0` | `5575.6 ms` |
| `python_functions` | `24/24` | `24/24` | `100.0` | `133.2 ms` |
| `llamaindex` | `24/24` | `24/24` | `100.0` | `1559.6 ms` |
| `specialist_supervisor` | `24/24` | `24/24` | `100.0` | `2812.6 ms` |

## By Slice

- `public`
  - `langgraph`: ok 24/24, keyword pass 24/24, quality 100.0, latency 2328.5ms
  - `crewai`: ok 24/24, keyword pass 24/24, quality 100.0, latency 5575.6ms
  - `python_functions`: ok 24/24, keyword pass 24/24, quality 100.0, latency 133.2ms
  - `llamaindex`: ok 24/24, keyword pass 24/24, quality 100.0, latency 1559.6ms
  - `specialist_supervisor`: ok 24/24, keyword pass 24/24, quality 100.0, latency 2812.6ms
...
- `langgraph`: status 200, latency 281.8ms, keyword pass `True`, quality `100`, reason `fato institucional canonico deve vir de fonte estruturada`
...
- `crewai`: status 200, latency 204.0ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
...
- `python_functions`: status 200, latency 125.5ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
  answer: O endereco publicado de Colegio Horizonte hoje e Rua das Acacias, 1450, Vila Mariana, Sao Paulo, SP, CEP 04567-120. O telefone principal hoje e (11) 3333-4200. O canal mais direto para falar com a secretaria hoje e o WhatsApp (11) 97500-2041.
- `llamaindex`: status 200, latency 142.4ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: O endereco publicado de Colegio Horizonte hoje e Rua das Acacias, 1450, Vila Mariana, Sao Paulo, SP, CEP 04567-120. O telefone principal hoje e (11) 3333-4200. O canal mais direto para falar com a secretaria hoje e o WhatsApp (11) 97500-2041.
- `specialist_supervisor`: status 200, latency 136.0ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_fast_path:contact_bundle`
...
- `langgraph`: status 200, latency 5400.5ms, keyword pass `True`, quality `100`, reason `fato institucional canonico deve vir de fonte estruturada`
...
- `crewai`: status 200, latency 196.6ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
...
- `python_functions`: status 200, latency 169.7ms, keyword pass `True`, quality `100`, reason `python_functions_native_structured:institution`
...
- `llamaindex`: status 200, latency 10967.9ms, keyword pass `True`, quality `100`, reason `llamaindex_public_profile`
  answer: O horario de Biblioteca Aurora hoje funciona assim: Atendimento ao publico de segunda a sexta, das 7h30 as 18h00.
...
- Ensino Medio (Manha): 07:15 as 12:50. Trilhas eletivas, monitorias e laboratorios no contraturno a partir das 14h00.
- Fundamental II e Ensino Medio (Integral opcional): 07:00 as 17:30. Inclui estudo orientado, almoco, oficinas, esportes e acompanhamento no contraturno.
- `specialist_supervisor`: status 200, latency 203.5ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_fast_path:library_hours`
...
- `langgraph`: status 200, latency 121.2ms, keyword pass `True`, quality `100`, reason `fato institucional canonico deve vir de fonte estruturada`
...
- `crewai`: status 200, latency 203.3ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
...
- Atendimento comercial / Admissoes: bot, admissions, whatsapp comercial ou visita guiada.
- Orientacao educacional: bot, orientacao educacional ou secretaria.
- `llamaindex`: status 200, latency 119.6ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Hoje estes sao os responsaveis e canais mais diretos por assunto:
- Diretora geral: Helena Martins. Canal institucional: direcao@colegiohorizonte.edu.br.
- Atendimento comercial / Admissoes: bot, admissions, whatsapp comercial ou visita guiada.
- Orientacao educacional: bot, orientacao educacional ou secretaria.
- `specialist_supervisor`: status 200, latency 133.8ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_fast_path:service_routing`
  answer: Hoje estes sao os responsaveis e canais mais diretos por assunto:
- Diretora geral: Helena Martins. Canal institucional: direcao@colegiohorizonte.edu.br.
- Atendimento comercial / Admissoes: bot, admissions, whatsapp comercial ou visita guiada.
- Orientacao educacional: bot, orientacao educacional ou secretaria.
...
- `crewai`: status 200, latency 196.1ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
...
- `python_functions`: status 200, latency 140.1ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
  answer: Se eu tivesse 30 segundos para resumir Colegio Horizonte, eu diria isto: Escola laica com Ensino Fundamental II e Ensino Medio, acompanhamento tutorial e trilhas academicas no contraturno. Os diferenciais publicados com mais clareza hoje passam por Tutoria academica e projeto de vida, Espaco Maker integrado ao curriculo, Acolhimento estruturado para familias e estudantes. A proposta pedagogica publicada combina Projeto pedagogico laico, com foco em aprendizagem por projetos, cultura digital responsavel, acompanhamento socioemocional e preparacao academica progressiva.. Na pratica, isso aparece em aprendizagem por projetos, acompanhamento mais proximo, estudo orientado e contraturno com referencias claras para familias.
- `llamaindex`: status 200, latency 140.5ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Se eu tivesse 30 segundos para resumir Colegio Horizonte, eu diria isto: Escola laica com Ensino Fundamental II e Ensino Medio, acompanhamento tutorial e trilhas academicas no contraturno. Os diferenciais publicados com mais clareza hoje passam por Tutoria academica e projeto de vida, Espaco Maker integrado ao curriculo, Acolhimento estruturado para familias e estudantes. A proposta pedagogica publicada combina Projeto pedagogico laico, com foco em aprendizagem por projetos, cultura digital responsavel, acompanhamento socioemocional e preparacao academica progressiva.. Na pratica, isso aparece em aprendizagem por projetos, acompanhamento mais proximo, estudo orientado e contraturno com referencias claras para familias.
- `specialist_supervisor`: status 200, latency 136.5ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_fast_path:public_pitch`
...
- `langgraph`: status 200, latency 128.5ms, keyword pass `True`, quality `100`, reason `a consulta exige autenticacao ou vinculo antes de qualquer acesso`
...
- `crewai`: status 200, latency 199.0ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
...
- `python_functions`: status 200, latency 162.9ms, keyword pass `True`, quality `100`, reason `a consulta exige autenticacao ou vinculo antes de qualquer acesso`
...
- `langgraph`: status 200, latency 5475.9ms, keyword pass `True`, quality `100`, reason `fato institucional canonico deve vir de fonte estruturada`
...
- `crewai`: status 200, latency 15720.3ms, keyword pass `True`, quality `100`, reason `crewai_public_flow_completed`
  answer: Não encontrei o endereço da escola nos documentos que tenho. Para obter essa informação, sugiro que você verifique os canais oficiais da escola ou entre em contato diretamente com eles.
- `python_functions`: status 200, latency 166.5ms, keyword pass `True`, quality `100`, reason `python_functions_native_structured:institution`
  answer: Colegio Horizonte fica em Rua das Acacias, 1450, Vila Mariana, Sao Paulo, SP, CEP 04567-120.
- `llamaindex`: status 200, latency 9620.5ms, keyword pass `True`, quality `100`, reason `llamaindex_public_profile`
  answer: Colegio Horizonte fica em Rua das Acacias, 1450, Vila Mariana, Sao Paulo, SP, CEP 04567-120.
- `specialist_supervisor`: status 200, latency 134.9ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_fast_path:district`
...
- `langgraph`: status 200, latency 119.4ms, keyword pass `True`, quality `100`, reason `fato institucional canonico deve vir de fonte estruturada`
  answer: Hoje estes sao os responsaveis e canais mais diretos por assunto:
- Atendimento comercial / Admissoes: bot, admissions, whatsapp comercial ou visita guiada.
- Financeiro: bot, financeiro, portal autenticado ou email institucional.
- Orientacao educacional: bot, orientacao educacional ou secretaria.
- `crewai`: status 200, latency 195.4ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
...
Para bullying, convivencia e apoio escolar, o canal indicado e a orientacao educacional. Orientacao educacional (telefone): (11) 3333-4202
- `python_functions`: status 200, latency 120.6ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
  answer: Hoje estes sao os responsaveis e canais mais diretos por assunto:
- Atendimento comercial / Admissoes: bot, admissions, whatsapp comercial ou visita guiada.
- Financeiro: bot, financeiro, portal autenticado ou email institucional.
- Orientacao educacional: bot, orientacao educacional ou secretaria.
...
- Financeiro: bot, financeiro, portal autenticado ou email institucional.
- Orientacao educacional: bot, orientacao educacional ou secretaria.
- `specialist_supervisor`: status 200, latency 140.1ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_fast_path:service_routing`
  answer: Hoje estes sao os responsaveis e canais mais diretos por assunto:
- Atendimento comercial / Admissoes: bot, admissions, whatsapp comercial ou visita guiada.
- Financeiro: bot, financeiro, portal autenticado ou email institucional.
- Orientacao educacional: bot, orientacao educacional ou secretaria.
...
- `langgraph`: status 200, latency 125.4ms, keyword pass `True`, quality `100`, reason `fato institucional canonico deve vir de fonte estruturada`
  answer: Hoje estes sao os responsaveis e canais mais diretos por assunto:
- Orientacao educacional: bot, orientacao educacional ou secretaria.
- Secretaria: bot, secretaria presencial, email institucional ou portal.
- `crewai`: status 200, latency 17958.3ms, keyword pass `True`, quality `100`, reason `crewai_public_flow_completed`
  answer: Os departamentos da escola têm funções bem definidas para cada fase do aprendizado! O Departamento de Educação Infantil cuida do desenvolvimento e aprendizado das crianças de 0 a 5 anos, em um ambiente seguro e estimulante. Já o Departamento de Ensino Fundamental atua na formação de alunos do 1º ao 9º ano, focando em habilidades essenciais e no preparo para o futuro. Por fim, o Departamento de Ensino Médio prepara os estudantes para o ensino superior e o mercado de trabalho, estimulando o pensamento crítico e a autonomia.
- `python_functions`: status 200, latency 122.3ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
  answer: Hoje estes sao os responsaveis e canais mais diretos por assunto:
- Orientacao educacional: bot, orientacao educacional ou secretaria.
- Secretaria: bot, secretaria presencial, email institucional ou portal.
- `llamaindex`: status 200, latency 106.5ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Hoje estes sao os responsaveis e canais mais diretos por assunto:
- Orientacao educacional: bot, orientacao educacional ou secretaria.
- Secretaria: bot, secretaria presencial, email institucional ou portal.
- `specialist_supervisor`: status 200, latency 157.0ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:human_handoff`
  answer: Encaminhei sua solicitacao para a fila de coordenacao. Protocolo: ATD-20260401-AFA845FC. Status atual: queued. Se preferir, voce tambem pode seguir pelo WhatsApp oficial (11) 97500-2040.

### Quando abre a matricula de 2026 e quando comecam as aulas?

- Slice: `public`
- Thread: `system_bank:timeline_e_calendario_publico:Q009` turn `1`
- `langgraph`: status 200, latency 5976.5ms, keyword pass `True`, quality `100`, reason `datas institucionais publicas devem vir de leitura estruturada e auditavel`
...
- `crewai`: status 200, latency 184.8ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
...
- `python_functions`: status 200, latency 114.9ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
...
- `llamaindex`: status 200, latency 108.7ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Hoje a base publica de Colegio Horizonte nao traz um marco institucional estruturado para essa data.
- `specialist_supervisor`: status 200, latency 135.0ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:public_timeline_bundle`
  answer: O ciclo publico de matricula para 2026 foi aberto em 6 de outubro de 2025, com pre-cadastro, visita orientada opcional e triagem inicial de documentos. O atendimento comercial segue enquanto houver vagas por segmento.
As aulas do Ensino Fundamental II e do Ensino Medio comecam em 2 de fevereiro de 2026. A semana de acolhimento do 6o ano e da primeira serie ocorre entre 2 e 6 de fevereiro.
...
- `langgraph`: status 200, latency 1485.0ms, keyword pass `True`, quality `100`, reason `a intencao esta ambigua e exige clarificacao antes de recuperar contexto`
  answer: Olá! No momento, não tenho informações sobre eventos públicos específicos para famílias e responsáveis nesta semana.

Para ficar por dentro de todas as novidades e o calendário completo de eventos, sugiro que consulte o nosso site oficial ou o portal do aluno, que são atualizados regularmente. Se preferir, também pode entrar em contato com a Secretaria Escolar para mais detalhes.
- `crewai`: status 200, latency 17304.5ms, keyword pass `True`, quality `100`, reason `crewai_public_flow_completed`
  answer: O único evento público que encontrei para famílias e responsáveis é uma visita guiada que acontecerá em 06/04/2026. Você gostaria de saber mais sobre este evento ou está procurando por outra data ou tipo de evento?
- `python_functions`: status 200, latency 174.4ms, keyword pass `True`, quality `100`, reason `a intencao esta ambigua e exige clarificacao antes de recuperar contexto`
...
- `llamaindex`: status 200, latency 6419.1ms, keyword pass `True`, quality `100`, reason `llamaindex_public_profile`
  answer: Hoje a base publica de eventos de Colegio Horizonte nao trouxe agenda estruturada para esse pedido.
- `specialist_supervisor`: status 200, latency 122.3ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:calendar_week`
...
- `langgraph`: status 200, latency 132.4ms, keyword pass `True`, quality `100`, reason `a consulta exige autenticacao ou vinculo antes de qualquer acesso`
  answer: Essa consulta depende de autenticacao e vinculo da sua conta no Telegram. Use o portal da escola para gerar o codigo de vinculacao e depois envie o comando `/start link_<codigo>` ao bot.
- `crewai`: status 200, latency 17831.8ms, keyword pass `True`, quality `100`, reason `crewai_public_flow_completed`
  answer: Olá! O calendário escolar de 2024 já está disponível. O primeiro dia de aula para todos os alunos será em 5 de fevereiro de 2024. O recesso de meio de ano acontecerá de 15 a 26 de julho de 2024, e o último dia de aula será em 13 de dezembro de 2024.
- `python_functions`: status 200, latency 127.4ms, keyword pass `True`, quality `100`, reason `a consulta exige autenticacao ou vinculo antes de qualquer acesso`
  answer: Essa consulta depende de autenticacao e vinculo da sua conta no Telegram. Use o portal da escola para gerar o codigo de vinculacao e depois envie o comando `/start link_<codigo>` ao bot.
- `llamaindex`: status 200, latency 131.1ms, keyword pass `True`, quality `100`, reason `a consulta exige autenticacao ou vinculo antes de qualquer acesso`
  answer: Essa consulta depende de autenticacao e vinculo da sua conta no Telegram. Use o portal da escola para gerar o codigo de vinculacao e depois envie o comando `/start link_<codigo>` ao bot.
- `specialist_supervisor`: status 200, latency 125.8ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:first_bimester_timeline`
...
- `langgraph`: status 200, latency 5650.5ms, keyword pass `True`, quality `100`, reason `datas institucionais publicas devem vir de leitura estruturada e auditavel`
...
- `crewai`: status 200, latency 192.0ms, keyword pass `True`, quality `100`, reason `crewai_protected_auth_required`
  answer: Essa consulta depende de autenticacao e vinculo da sua conta no Telegram. Use o portal da escola para gerar o codigo de vinculacao e depois envie o comando /start link_<codigo> ao bot.
- `python_functions`: status 200, latency 117.7ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
  answer: As aulas do Ensino Fundamental II e do Ensino Medio comecam em 2 de fevereiro de 2026. A semana de acolhimento do 6o ano e da primeira serie ocorre entre 2 e 6 de fevereiro.
- `llamaindex`: status 200, latency 106.6ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Hoje a base publica de Colegio Horizonte nao traz um marco institucional estruturado para essa data.
- `specialist_supervisor`: status 200, latency 130.2ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:eval_calendar`
...
- `langgraph`: status 200, latency 6098.4ms, keyword pass `True`, quality `100`, reason `datas institucionais publicas devem vir de leitura estruturada e auditavel`
...
- `crewai`: status 200, latency 191.4ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
  answer: As aulas do Ensino Fundamental II e do Ensino Medio comecam em 2 de fevereiro de 2026.
- `python_functions`: status 200, latency 113.9ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
...
- `llamaindex`: status 200, latency 110.9ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Hoje a base publica de Colegio Horizonte nao traz um marco institucional estruturado para essa data.
- `specialist_supervisor`: status 200, latency 137.7ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:public_timeline`
  answer: As aulas do Ensino Fundamental II e do Ensino Medio comecam em 2 de fevereiro de 2026. A semana de acolhimento do 6o ano e da primeira serie ocorre entre 2 e 6 de fevereiro.
...
- `langgraph`: status 200, latency 5152.3ms, keyword pass `True`, quality `100`, reason `datas institucionais publicas devem vir de leitura estruturada e auditavel`
  answer: Que ótima iniciativa planejar com antecedência! Para que sua viagem não atrapalhe os estudos, sugiro que você observe as datas de início das aulas, que em 2026 será em 2 de fevereiro, e a cerimônia de conclusão do Ensino Fundamental II, marcada para 12 de dezembro. O ciclo de matrículas para 2026 também começou em 6 de outubro de 2025, caso precise dessa informação.
- `crewai`: status 200, latency 20249.1ms, keyword pass `True`, quality `100`, reason `crewai_public_flow_completed`
...
- `python_functions`: status 200, latency 125.3ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
...
- `llamaindex`: status 200, latency 117.2ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Hoje a base publica de Colegio Horizonte nao traz um marco institucional estruturado para essa data.
- `specialist_supervisor`: status 200, latency 138.7ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:travel_planning`
  answer: Para planejar uma viagem sem atrapalhar a vida escolar, vale observar estes marcos publicos antes de fechar datas:
- O ciclo publico de matricula para 2026 foi aberto em 6 de outubro de 2025, com pre-cadastro, visita orientada opcional e triagem inicial de documentos.
- As aulas do Ensino Fundamental II e do Ensino Medio comecam em 2 de fevereiro de 2026.
- A cerimonia interna de conclusao do Ensino Fundamental II esta prevista para 12 de dezembro de 2026, no fim da tarde, apos o encerramento do ano letivo.
...
- `langgraph`: status 200, latency 5437.5ms, keyword pass `True`, quality `100`, reason `planejamento semantico publico encontrou um ato estruturado mais adequado ao turno`
  answer: Olá! No nosso calendário, os marcos institucionais e eventos coletivos são sempre abertos para toda a comunidade. Já os detalhes individuais de cada aluno, convites específicos, informações financeiras e acompanhamentos são acessíveis apenas com autenticação, para garantir a segurança e privacidade. Fique tranquilo(a), qualquer atualização importante no calendário de avaliações será comunicada prontamente pelos nossos canais oficiais e no portal.
- `crewai`: status 200, latency 20250.4ms, keyword pass `True`, quality `100`, reason `crewai_public_flow_completed`
  answer: Nao consegui concluir essa resposta agora pelo caminho principal configurado. Tente reformular em uma frase mais direta ou repetir em instantes.
...
- `llamaindex`: status 200, latency 114.7ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: No calendario publico, o que costuma ficar aberto para familias sao marcos institucionais gerais e eventos coletivos. O que depende de autenticacao ou contexto interno sao detalhes individuais por aluno, convites direcionados, protocolos, situacoes financeiras e acompanhamentos protegidos. Sempre que houver ajuste relevante no calendario de avaliacao, a escola atualiza o portal e reforca a comunicacao pelos canais oficiais. O ambiente digital amplia o atendimento, mas nao substitui todos os processos formais.
- `specialist_supervisor`: status 200, latency 130.5ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:restricted_document_denied`
...
- `langgraph`: status 200, latency 5572.7ms, keyword pass `True`, quality `100`, reason `datas institucionais publicas devem vir de leitura estruturada e auditavel`
...
- `crewai`: status 200, latency 20248.3ms, keyword pass `True`, quality `100`, reason `crewai_public_flow_timeout`
  answer: Os documentos fornecidos não contêm informações sobre a fase de "fechamento" do ano escolar. Você gostaria de saber sobre algum aspecto específico do encerramento do ano letivo, como formaturas, avaliações finais ou procedimentos de fim de ano?
...
- `llamaindex`: status 200, latency 180.4ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Hoje a base publica de Colegio Horizonte nao traz um marco institucional estruturado para essa data.
- `specialist_supervisor`: status 200, latency 137.0ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:year_three_phases`
  answer: Admissao: O ciclo publico de matricula para 2026 foi aberto em 6 de outubro de 2025, com pre-cadastro, visita orientada opcional e triagem inicial de documentos.
...
- `langgraph`: status 200, latency 147.7ms, keyword pass `True`, quality `100`, reason `o turno atual trouxe pistas explicitas de outro dominio protegido e o roteamento foi corrigido`
...
- `crewai`: status 200, latency 635.2ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
  answer: Hoje os documentos exigidos para a matricula publicados pela escola sao:
- Formulario cadastral preenchido
- Documento de identificacao do aluno
- CPF do aluno, quando houver
...
- Documento de identificacao do responsavel legal
Se quiser, eu tambem posso explicar as etapas da matricula ou como funciona o envio inicial desses documentos.
- `llamaindex`: status 200, latency 140.9ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Hoje os documentos exigidos para a matricula publicados pela escola sao:
- Formulario cadastral preenchido
- Documento de identificacao do aluno
- CPF do aluno, quando houver
...
- Documento de identificacao do responsavel legal
Se quiser, eu tambem posso explicar as etapas da matricula ou como funciona o envio inicial desses documentos.
- `specialist_supervisor`: status 200, latency 137.6ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:admissions_documents`
...
- `langgraph`: status 200, latency 119.5ms, keyword pass `True`, quality `100`, reason `fato institucional canonico deve vir de fonte estruturada`
...
- `crewai`: status 200, latency 722.7ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
...
- `python_functions`: status 200, latency 144.0ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
  answer: No Colegio Horizonte, a referencia publica de aprovacao e media 7,0/10. Quando a media fica abaixo da referencia, a escola combina monitorias, plantoes e orientacoes de recuperacao conforme o calendario pedagogico. A referencia publica de aprovacao usada nas orientacoes do colegio e 7,0, com fechamento final sempre acompanhado pela equipe pedagogica no periodo letivo.
- `llamaindex`: status 200, latency 157.0ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: No Colegio Horizonte, a referencia publica de aprovacao e media 7,0/10. Quando a media fica abaixo da referencia, a escola combina monitorias, plantoes e orientacoes de recuperacao conforme o calendario pedagogico. A referencia publica de aprovacao usada nas orientacoes do colegio e 7,0, com fechamento final sempre acompanhado pela equipe pedagogica no periodo letivo.
- `specialist_supervisor`: status 200, latency 12119.2ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_direct:academic_specialist`
  answer: No Colégio Horizonte, a política de avaliação e promoção exige uma média mínima de 7,0 em uma escala de 0 a 10. Quando a média de um estudante fica abaixo dessa referência, a escola oferece suporte de recuperação que inclui monitorias, plantões e orientações pedagógicas, conforme o calendário. A aprovação final é sempre acompanhada pela equipe pedagógica durante o período letivo.
...
- `langgraph`: status 200, latency 126.3ms, keyword pass `True`, quality `100`, reason `fato institucional canonico deve vir de fonte estruturada`
...
- `crewai`: status 200, latency 199.0ms, keyword pass `True`, quality `100`, reason `crewai_protected_auth_required`
  answer: Essa consulta depende de autenticacao e vinculo da sua conta no Telegram. Use o portal da escola para gerar o codigo de vinculacao e depois envie o comando /start link_<codigo> ao bot.
- `python_functions`: status 200, latency 125.1ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
  answer: No Colegio Horizonte, a referencia publica minima de frequencia e 75,0% por componente. Se a frequencia de um componente cair abaixo de 75%, o estudante entra em alerta academico e a coordenacao pode acionar a familia. A permanencia abaixo desse limite pode comprometer a aprovacao por frequencia. A escola acompanha justificativas, recorrencia e necessidade de plano de recomposicao junto a familia e ao estudante.
- `llamaindex`: status 200, latency 134.2ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: No Colegio Horizonte, a referencia publica minima de frequencia e 75,0% por componente. Se a frequencia de um componente cair abaixo de 75%, o estudante entra em alerta academico e a coordenacao pode acionar a familia. A permanencia abaixo desse limite pode comprometer a aprovacao por frequencia. A escola acompanha justificativas, recorrencia e necessidade de plano de recomposicao junto a familia e ao estudante.
- `specialist_supervisor`: status 200, latency 138.2ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:conduct_frequency_punctuality`
  answer: A politica publica de frequencia reforca presenca minima de 75,0% por componente e alerta academico quando a recorrencia de faltas compromete a aprovacao. Se a frequencia de um componente cair abaixo de 75%, o estudante entra em alerta academico e a coordenacao pode acionar a familia.
...
- `langgraph`: status 200, latency 5875.8ms, keyword pass `True`, quality `100`, reason `fato institucional canonico deve vir de fonte estruturada`
  answer: Sim. O Colegio Horizonte tem a Biblioteca Aurora. Atendimento ao publico de segunda a sexta, das 7h30 as 18h00.
- `crewai`: status 200, latency 182.1ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
  answer: Sim. Biblioteca Aurora: biblioteca   Biblioteca Aurora   service   True   Atendimento ao publico de segunda a sexta, das 7h30 as 18h00..
- `python_functions`: status 200, latency 130.3ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
  answer: A biblioteca se chama Biblioteca Aurora e funciona de segunda a sexta, das 7h30 as 18h00.
- `llamaindex`: status 200, latency 7835.1ms, keyword pass `True`, quality `100`, reason `llamaindex_public_profile`
  answer: Sim. O Colegio Horizonte tem a Biblioteca Aurora. Atendimento ao publico de segunda a sexta, das 7h30 as 18h00.
- `specialist_supervisor`: status 200, latency 135.1ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_fast_path:library_hours`
  answer: A biblioteca se chama Biblioteca Aurora e funciona Atendimento ao publico de segunda a sexta, das 7h30 as 18h00.
...
- `langgraph`: status 200, latency 148.5ms, keyword pass `True`, quality `100`, reason `fato institucional canonico deve vir de fonte estruturada`
...
- `crewai`: status 200, latency 149.3ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
...
No processo de matricula, o envio inicial pode ser digital. A validacao final continua com a secretaria.
O chat aberto nao deve receber documentos completos ou dados sensiveis. Quando possivel, use o portal autenticado ou o email institucional.
- `llamaindex`: status 200, latency 126.3ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Sim. O envio inicial de documentos pode ser feito por canal digital.
Hoje os canais mais diretos publicados para isso sao:
- portal institucional
- email da secretaria
...
No processo de matricula, o envio inicial pode ser digital. A validacao final continua com a secretaria.
O chat aberto nao deve receber documentos completos ou dados sensiveis. Quando possivel, use o portal autenticado ou o email institucional.
- `specialist_supervisor`: status 200, latency 129.5ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:document_submission_policy`
...
- `langgraph`: status 200, latency 196.4ms, keyword pass `True`, quality `100`, reason `fato institucional canonico deve vir de fonte estruturada`
...
- `crewai`: status 200, latency 177.6ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
...
- `python_functions`: status 200, latency 122.2ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
...
O chat aberto nao deve receber documentos completos ou dados sensiveis. Quando possivel, use o portal autenticado ou o email institucional.
- `llamaindex`: status 200, latency 126.8ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Hoje a familia precisa entender quatro frentes publicas deste fluxo:
- Secretaria: recebe declaracoes, historico, atualizacoes cadastrais e orientacoes administrativas.
- Portal institucional: centraliza protocolo e envio digital inicial de documentos.
- Credenciais: login e senha do portal continuam sendo a base de acesso; se precisar recuperar acesso, o melhor caminho e a secretaria ou o suporte digital.
- Documentos: o envio inicial pode ser feito por portal institucional, email da secretaria ou secretaria presencial.
O chat aberto nao deve receber documentos completos ou dados sensiveis. Quando possivel, use o portal autenticado ou o email institucional.
- `specialist_supervisor`: status 200, latency 134.4ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_fast_path:service_credentials_bundle`
  answer: Hoje a familia precisa entender quatro frentes publicas deste fluxo:
- Secretaria: recebe declaracoes, historico, atualizacoes cadastrais e orientacoes administrativas.
- Portal institucional: centraliza protocolo e envio digital inicial de documentos.
...
- `langgraph`: status 200, latency 134.0ms, keyword pass `True`, quality `100`, reason `fato institucional canonico deve vir de fonte estruturada`
...
- `python_functions`: status 200, latency 116.5ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
  answer: O edital disciplina pedidos de bolsa social, desconto institucional, politica para irmaos e condicoes de manutencao do beneficio no ano letivo de 2026. Pedidos devem ser feitos dentro da janela informada pela escola, com envio de documentacao socioeconomica e demais comprovantes solicitados no regulamento especifico. A concessao de bolsas e descontos depende de analise documental, criterios internos, disponibilidade orcamentaria e validacao administrativa. Beneficios podem depender de cumprimento das condicoes previstas, como adimplencia, atualizacao cadastral e apresentacao tempestiva de documentos complementares quando solicitados. A rematricula para estudantes ja vinculados ao Colegio Horizonte ocorre em janela administrativa propria, divulgada no portal e reforcada pelos canais oficiais. Pedidos de transferencia para ingresso durante o ano letivo dependem de vaga, analise academica, conferencia documental e alinhamento com a serie pretendida. Quando a familia deseja transferir o aluno para outra instituicao, a solicitacao deve ser formalizada junto a secretaria. Pedidos de cancelamento devem ser apresentados pelo responsavel legal por canal oficial e podem exigir confirmacao adicional da secretaria ou do financeiro. Declaracoes simples costumam ser liberadas em ate dois dias uteis.
- `llamaindex`: status 200, latency 118.0ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: O edital disciplina pedidos de bolsa social, desconto institucional, politica para irmaos e condicoes de manutencao do beneficio no ano letivo de 2026. Pedidos devem ser feitos dentro da janela informada pela escola, com envio de documentacao socioeconomica e demais comprovantes solicitados no regulamento especifico. A concessao de bolsas e descontos depende de analise documental, criterios internos, disponibilidade orcamentaria e validacao administrativa. Beneficios podem depender de cumprimento das condicoes previstas, como adimplencia, atualizacao cadastral e apresentacao tempestiva de documentos complementares quando solicitados. A rematricula para estudantes ja vinculados ao Colegio Horizonte ocorre em janela administrativa propria, divulgada no portal e reforcada pelos canais oficiais. Pedidos de transferencia para ingresso durante o ano letivo dependem de vaga, analise academica, conferencia documental e alinhamento com a serie pretendida. Quando a familia deseja transferir o aluno para outra instituicao, a solicitacao deve ser formalizada junto a secretaria. Pedidos de cancelamento devem ser apresentados pelo responsavel legal por canal oficial e podem exigir confirmacao adicional da secretaria ou do financeiro. Declaracoes simples costumam ser liberadas em ate dois dias uteis.
- `specialist_supervisor`: status 200, latency 32549.3ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_manager_judge:institution:structured_tools`
  answer: Sua solicitação sobre o funcionamento de bolsas, descontos, e regras de rematrícula, transferência e cancelamento foi encaminhada à nossa equipe de atendimento. Em breve, um especialista entrará em contato para fornecer todas as informações necessárias. Seu protocolo de atendimento é ATD-20260401-8846CED9.
...
- `langgraph`: status 200, latency 1846.8ms, keyword pass `True`, quality `100`, reason `planejamento semantico publico encontrou um ato estruturado mais adequado ao turno`
...
- `crewai`: status 200, latency 213.9ms, keyword pass `True`, quality `100`, reason `crewai_protected_auth_required`
  answer: Essa consulta depende de autenticacao e vinculo da sua conta no Telegram. Use o portal da escola para gerar o codigo de vinculacao e depois envie o comando /start link_<codigo> ao bot.
- `python_functions`: status 200, latency 122.7ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
...
- `llamaindex`: status 200, latency 111.7ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Atestados medicos e justificativas formais podem ser usados para apoiar analise de ausencia, segunda chamada ou adaptacao temporaria de rotina. Pedidos de segunda chamada exigem justificativa no prazo orientado pela escola.
- `specialist_supervisor`: status 200, latency 10086.4ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_direct:academic_specialist`
  answer: A política acadêmica geral não detalha o procedimento específico para o que você deve fazer ao faltar em uma prova por motivo de saúde. No entanto, a escola acompanha justificativas, recorrência e necessidade de plano de recomposição junto à família e ao estudante.

Para obter informações precisas sobre o que fazer ao perder uma prova por motivo de saúde, incluindo a possibilidade de uma segunda chamada ou entrega de atestado médico, recomendo que você entre em contato diretamente com a **Coordenação Pedagógica** ou a **Orientação Educacional** do Colégio Horizonte. Eles poderão fornecer as instruções específicas e o calendário para esses casos.
