...
Date: 2026-04-01T16:51:43.061239+00:00

Dataset: `../../tests/evals/datasets/system_question_bank_wave_public_grounding.json`

Run prefix: `debug:five-path:20260401T164740Z`

## Stack Summary

| Stack | OK | Keyword pass | Quality | Avg latency |
| --- | --- | --- | --- | --- |
| `langgraph` | `24/24` | `24/24` | `100.0` | `2314.9 ms` |
| `crewai` | `24/24` | `24/24` | `100.0` | `5945.7 ms` |
| `python_functions` | `24/24` | `24/24` | `100.0` | `149.1 ms` |
| `llamaindex` | `24/24` | `24/24` | `100.0` | `1457.9 ms` |
| `specialist_supervisor` | `24/24` | `24/24` | `100.0` | `126.0 ms` |

## By Slice

- `public`
  - `langgraph`: ok 24/24, keyword pass 24/24, quality 100.0, latency 2314.9ms
  - `crewai`: ok 24/24, keyword pass 24/24, quality 100.0, latency 5945.7ms
  - `python_functions`: ok 24/24, keyword pass 24/24, quality 100.0, latency 149.1ms
  - `llamaindex`: ok 24/24, keyword pass 24/24, quality 100.0, latency 1457.9ms
  - `specialist_supervisor`: ok 24/24, keyword pass 24/24, quality 100.0, latency 126.0ms
...
- `langgraph`: status 200, latency 193.4ms, keyword pass `True`, quality `100`, reason `fato institucional canonico deve vir de fonte estruturada`
...
- `crewai`: status 200, latency 232.6ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
...
- `python_functions`: status 200, latency 238.9ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
  answer: O endereco publicado de Colegio Horizonte hoje e Rua das Acacias, 1450, Vila Mariana, Sao Paulo, SP, CEP 04567-120. O telefone principal hoje e (11) 3333-4200. O canal mais direto para falar com a secretaria hoje e o WhatsApp (11) 97500-2041.
- `llamaindex`: status 200, latency 159.7ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: O endereco publicado de Colegio Horizonte hoje e Rua das Acacias, 1450, Vila Mariana, Sao Paulo, SP, CEP 04567-120. O telefone principal hoje e (11) 3333-4200. O canal mais direto para falar com a secretaria hoje e o WhatsApp (11) 97500-2041.
- `specialist_supervisor`: status 200, latency 145.7ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_fast_path:contact_bundle`
...
- `langgraph`: status 200, latency 5812.7ms, keyword pass `True`, quality `100`, reason `fato institucional canonico deve vir de fonte estruturada`
  answer: Que bom que você perguntou! Sim, temos uma biblioteca de verdade, a Biblioteca Aurora. Ela funciona de segunda a sexta-feira, das 7h30 às 18h00, e está sempre de portas abertas para nossos alunos.
- `crewai`: status 200, latency 237.6ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
...
- `python_functions`: status 200, latency 188.0ms, keyword pass `True`, quality `100`, reason `python_functions_native_structured:institution`
...
- `llamaindex`: status 200, latency 11838.9ms, keyword pass `True`, quality `100`, reason `llamaindex_public_profile`
  answer: O horario de Biblioteca Aurora hoje funciona assim: Atendimento ao publico de segunda a sexta, das 7h30 as 18h00.
...
- Ensino Medio (Manha): 07:15 as 12:50. Trilhas eletivas, monitorias e laboratorios no contraturno a partir das 14h00.
- Fundamental II e Ensino Medio (Integral opcional): 07:00 as 17:30. Inclui estudo orientado, almoco, oficinas, esportes e acompanhamento no contraturno.
- `specialist_supervisor`: status 200, latency 123.3ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_fast_path:library_hours`
...
- `langgraph`: status 200, latency 131.2ms, keyword pass `True`, quality `100`, reason `fato institucional canonico deve vir de fonte estruturada`
...
- `crewai`: status 200, latency 193.1ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
...
- Atendimento comercial / Admissoes: bot, admissions, whatsapp comercial ou visita guiada.
- Orientacao educacional: bot, orientacao educacional ou secretaria.
- `llamaindex`: status 200, latency 113.6ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Hoje estes sao os responsaveis e canais mais diretos por assunto:
- Diretora geral: Helena Martins. Canal institucional: direcao@colegiohorizonte.edu.br.
- Atendimento comercial / Admissoes: bot, admissions, whatsapp comercial ou visita guiada.
- Orientacao educacional: bot, orientacao educacional ou secretaria.
- `specialist_supervisor`: status 200, latency 116.1ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_fast_path:service_routing`
  answer: Hoje estes sao os responsaveis e canais mais diretos por assunto:
- Diretora geral: Helena Martins. Canal institucional: direcao@colegiohorizonte.edu.br.
- Atendimento comercial / Admissoes: bot, admissions, whatsapp comercial ou visita guiada.
- Orientacao educacional: bot, orientacao educacional ou secretaria.
...
- `langgraph`: status 200, latency 132.1ms, keyword pass `True`, quality `100`, reason `fato institucional canonico deve vir de fonte estruturada`
...
- `crewai`: status 200, latency 284.5ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
...
- `python_functions`: status 200, latency 132.5ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
  answer: Se eu tivesse 30 segundos para resumir Colegio Horizonte, eu diria isto: Escola laica com Ensino Fundamental II e Ensino Medio, acompanhamento tutorial e trilhas academicas no contraturno. Os diferenciais publicados com mais clareza hoje passam por Tutoria academica e projeto de vida, Espaco Maker integrado ao curriculo, Acolhimento estruturado para familias e estudantes. A proposta pedagogica publicada combina Projeto pedagogico laico, com foco em aprendizagem por projetos, cultura digital responsavel, acompanhamento socioemocional e preparacao academica progressiva.. Na pratica, isso aparece em aprendizagem por projetos, acompanhamento mais proximo, estudo orientado e contraturno com referencias claras para familias.
- `llamaindex`: status 200, latency 132.1ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Se eu tivesse 30 segundos para resumir Colegio Horizonte, eu diria isto: Escola laica com Ensino Fundamental II e Ensino Medio, acompanhamento tutorial e trilhas academicas no contraturno. Os diferenciais publicados com mais clareza hoje passam por Tutoria academica e projeto de vida, Espaco Maker integrado ao curriculo, Acolhimento estruturado para familias e estudantes. A proposta pedagogica publicada combina Projeto pedagogico laico, com foco em aprendizagem por projetos, cultura digital responsavel, acompanhamento socioemocional e preparacao academica progressiva.. Na pratica, isso aparece em aprendizagem por projetos, acompanhamento mais proximo, estudo orientado e contraturno com referencias claras para familias.
- `specialist_supervisor`: status 200, latency 122.0ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_fast_path:public_pitch`
...
- `langgraph`: status 200, latency 143.1ms, keyword pass `True`, quality `100`, reason `a consulta exige autenticacao ou vinculo antes de qualquer acesso`
...
- `crewai`: status 200, latency 222.3ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
...
- `python_functions`: status 200, latency 141.9ms, keyword pass `True`, quality `100`, reason `a consulta exige autenticacao ou vinculo antes de qualquer acesso`
...
- `llamaindex`: status 200, latency 140.4ms, keyword pass `True`, quality `100`, reason `a consulta exige autenticacao ou vinculo antes de qualquer acesso`
  answer: Essa consulta depende de autenticacao e vinculo da sua conta no Telegram. Use o portal da escola para gerar o codigo de vinculacao e depois envie o comando `/start link_<codigo>` ao bot.
- `specialist_supervisor`: status 200, latency 105.0ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_llm_unconfigured`
...
- `langgraph`: status 200, latency 5505.1ms, keyword pass `True`, quality `100`, reason `fato institucional canonico deve vir de fonte estruturada`
...
- `crewai`: status 200, latency 19902.2ms, keyword pass `True`, quality `100`, reason `crewai_public_flow_completed`
  answer: A escola está localizada na Rua do Carmo, 76 - Centro, Rio de Janeiro - RJ.
...
- `llamaindex`: status 200, latency 8962.9ms, keyword pass `True`, quality `100`, reason `llamaindex_public_profile`
  answer: Colegio Horizonte fica em Rua das Acacias, 1450, Vila Mariana, Sao Paulo, SP, CEP 04567-120.
- `specialist_supervisor`: status 200, latency 126.4ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_fast_path:district`
...
- `langgraph`: status 200, latency 150.3ms, keyword pass `True`, quality `100`, reason `fato institucional canonico deve vir de fonte estruturada`
  answer: Hoje estes sao os responsaveis e canais mais diretos por assunto:
- Atendimento comercial / Admissoes: bot, admissions, whatsapp comercial ou visita guiada.
- Financeiro: bot, financeiro, portal autenticado ou email institucional.
- Orientacao educacional: bot, orientacao educacional ou secretaria.
- `crewai`: status 200, latency 236.4ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
...
Para bullying, convivencia e apoio escolar, o canal indicado e a orientacao educacional. Orientacao educacional (telefone): (11) 3333-4202
- `python_functions`: status 200, latency 144.9ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
  answer: Hoje estes sao os responsaveis e canais mais diretos por assunto:
- Atendimento comercial / Admissoes: bot, admissions, whatsapp comercial ou visita guiada.
- Financeiro: bot, financeiro, portal autenticado ou email institucional.
- Orientacao educacional: bot, orientacao educacional ou secretaria.
- `llamaindex`: status 200, latency 139.5ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Hoje estes sao os responsaveis e canais mais diretos por assunto:
- Atendimento comercial / Admissoes: bot, admissions, whatsapp comercial ou visita guiada.
- Financeiro: bot, financeiro, portal autenticado ou email institucional.
- Orientacao educacional: bot, orientacao educacional ou secretaria.
- `specialist_supervisor`: status 200, latency 122.6ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_fast_path:service_routing`
  answer: Hoje estes sao os responsaveis e canais mais diretos por assunto:
- Atendimento comercial / Admissoes: bot, admissions, whatsapp comercial ou visita guiada.
- Financeiro: bot, financeiro, portal autenticado ou email institucional.
- Orientacao educacional: bot, orientacao educacional ou secretaria.
...
- `crewai`: status 200, latency 16596.1ms, keyword pass `True`, quality `100`, reason `crewai_public_flow_completed`
  answer: Os departamentos escolares, como o de Ensino Fundamental, Educação Infantil e Educação Especial, têm como responsabilidade coordenar, acompanhar e avaliar as atividades pedagógicas, promover a formação continuada dos profissionais da educação, garantir a qualidade do ensino e desenvolver projetos e programas educacionais. O Departamento de Educação Especial também se dedica a assegurar a inclusão de alunos com necessidades educacionais especiais.
- `python_functions`: status 200, latency 124.5ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
  answer: Hoje estes sao os responsaveis e canais mais diretos por assunto:
- Orientacao educacional: bot, orientacao educacional ou secretaria.
- Secretaria: bot, secretaria presencial, email institucional ou portal.
- `llamaindex`: status 200, latency 118.8ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Hoje estes sao os responsaveis e canais mais diretos por assunto:
- Orientacao educacional: bot, orientacao educacional ou secretaria.
- Secretaria: bot, secretaria presencial, email institucional ou portal.
- `specialist_supervisor`: status 200, latency 152.4ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:human_handoff`
  answer: Encaminhei sua solicitacao para a fila de coordenacao. Protocolo: ATD-20260401-A5B3EA0C. Status atual: queued. Se preferir, voce tambem pode seguir pelo WhatsApp oficial (11) 97500-2040.

### Quando abre a matricula de 2026 e quando comecam as aulas?

- Slice: `public`
- Thread: `system_bank:timeline_e_calendario_publico:Q009` turn `1`
- `langgraph`: status 200, latency 5807.9ms, keyword pass `True`, quality `100`, reason `datas institucionais publicas devem vir de leitura estruturada e auditavel`
  answer: Que bom que você está se planejando com antecedência! As matrículas para 2026 no Colégio Horizonte abriram em 6 de outubro de 2025, com pré-cadastro e visita orientada. As aulas para o Ensino Fundamental II e Ensino Médio começam em 2 de fevereiro de 2026, e teremos uma semana especial de acolhimento para o 6º ano e a 1ª série entre 2 e 6 de fevereiro.
- `crewai`: status 200, latency 197.3ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
...
- `python_functions`: status 200, latency 127.2ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
...
- `llamaindex`: status 200, latency 118.7ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Hoje a base publica de Colegio Horizonte nao traz um marco institucional estruturado para essa data.
- `specialist_supervisor`: status 200, latency 120.8ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:public_timeline_bundle`
  answer: O ciclo publico de matricula para 2026 foi aberto em 6 de outubro de 2025, com pre-cadastro, visita orientada opcional e triagem inicial de documentos. O atendimento comercial segue enquanto houver vagas por segmento.
As aulas do Ensino Fundamental II e do Ensino Medio comecam em 2 de fevereiro de 2026. A semana de acolhimento do 6o ano e da primeira serie ocorre entre 2 e 6 de fevereiro.
...
- `langgraph`: status 200, latency 1937.0ms, keyword pass `True`, quality `100`, reason `a intencao esta ambigua e exige clarificacao antes de recuperar contexto`
  answer: Olá! No momento, não tenho informações sobre eventos públicos específicos para famílias e responsáveis nesta semana.

Para ficar por dentro de toda a nossa programação, sugiro que você consulte o calendário oficial da escola ou entre em contato com a Secretaria Escolar. Eles poderão te ajudar com as datas e detalhes dos próximos eventos.
- `crewai`: status 200, latency 20283.6ms, keyword pass `True`, quality `100`, reason `crewai_public_flow_timeout`
  answer: Olá! Para famílias e responsáveis, a única informação de evento público que encontrei é uma visita guiada em 06/04/2026. Você gostaria de saber sobre eventos futuros ou há algo específico que procura para esta semana?
- `python_functions`: status 200, latency 208.2ms, keyword pass `True`, quality `100`, reason `a intencao esta ambigua e exige clarificacao antes de recuperar contexto`
...
- `llamaindex`: status 200, latency 6266.9ms, keyword pass `True`, quality `100`, reason `llamaindex_public_profile`
  answer: Hoje a base publica de eventos de Colegio Horizonte nao trouxe agenda estruturada para esse pedido.
- `specialist_supervisor`: status 200, latency 109.8ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:calendar_week`
...
- `langgraph`: status 200, latency 136.4ms, keyword pass `True`, quality `100`, reason `a consulta exige autenticacao ou vinculo antes de qualquer acesso`
  answer: Essa consulta depende de autenticacao e vinculo da sua conta no Telegram. Use o portal da escola para gerar o codigo de vinculacao e depois envie o comando `/start link_<codigo>` ao bot.
- `crewai`: status 200, latency 20294.6ms, keyword pass `True`, quality `100`, reason `crewai_public_flow_timeout`
  answer: Olá! O calendário escolar de 2024 já está disponível. As aulas começam no dia 05 de fevereiro de 2024, o recesso de meio de ano será de 15 a 26 de julho de 2024 e o término das aulas está previsto para 13 de dezembro de 2024.
- `python_functions`: status 200, latency 137.9ms, keyword pass `True`, quality `100`, reason `a consulta exige autenticacao ou vinculo antes de qualquer acesso`
  answer: Essa consulta depende de autenticacao e vinculo da sua conta no Telegram. Use o portal da escola para gerar o codigo de vinculacao e depois envie o comando `/start link_<codigo>` ao bot.
- `llamaindex`: status 200, latency 197.5ms, keyword pass `True`, quality `100`, reason `a consulta exige autenticacao ou vinculo antes de qualquer acesso`
  answer: Essa consulta depende de autenticacao e vinculo da sua conta no Telegram. Use o portal da escola para gerar o codigo de vinculacao e depois envie o comando `/start link_<codigo>` ao bot.
- `specialist_supervisor`: status 200, latency 108.2ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:first_bimester_timeline`
...
- `langgraph`: status 200, latency 5667.7ms, keyword pass `True`, quality `100`, reason `datas institucionais publicas devem vir de leitura estruturada e auditavel`
...
- `crewai`: status 200, latency 198.0ms, keyword pass `True`, quality `100`, reason `crewai_protected_auth_required`
  answer: Essa consulta depende de autenticacao e vinculo da sua conta no Telegram. Use o portal da escola para gerar o codigo de vinculacao e depois envie o comando /start link_<codigo> ao bot.
- `python_functions`: status 200, latency 120.3ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
  answer: As aulas do Ensino Fundamental II e do Ensino Medio comecam em 2 de fevereiro de 2026. A semana de acolhimento do 6o ano e da primeira serie ocorre entre 2 e 6 de fevereiro.
- `llamaindex`: status 200, latency 123.1ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Hoje a base publica de Colegio Horizonte nao traz um marco institucional estruturado para essa data.
- `specialist_supervisor`: status 200, latency 173.0ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:eval_calendar`
...
- `langgraph`: status 200, latency 5316.8ms, keyword pass `True`, quality `100`, reason `datas institucionais publicas devem vir de leitura estruturada e auditavel`
...
Após o início das aulas, que para o Ensino Fundamental II e Ensino Médio será em 2 de fevereiro de 2026, teremos uma semana especial de acolhimento para o 6º ano e a 1ª série, entre os dias 2 e 6 de fevereiro.
- `crewai`: status 200, latency 206.6ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
  answer: As aulas do Ensino Fundamental II e do Ensino Medio comecam em 2 de fevereiro de 2026.
- `python_functions`: status 200, latency 121.8ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
...
- `llamaindex`: status 200, latency 118.7ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Hoje a base publica de Colegio Horizonte nao traz um marco institucional estruturado para essa data.
- `specialist_supervisor`: status 200, latency 124.3ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:public_timeline`
  answer: As aulas do Ensino Fundamental II e do Ensino Medio comecam em 2 de fevereiro de 2026. A semana de acolhimento do 6o ano e da primeira serie ocorre entre 2 e 6 de fevereiro.
...
- `langgraph`: status 200, latency 3976.6ms, keyword pass `True`, quality `100`, reason `datas institucionais publicas devem vir de leitura estruturada e auditavel`
...
- `crewai`: status 200, latency 20268.3ms, keyword pass `True`, quality `100`, reason `crewai_public_flow_timeout`
  answer: Não encontrei o calendário acadêmico nas informações que tenho. Para ter acesso a ele, sugiro que você verifique o site oficial da escola ou entre em contato diretamente com a secretaria. Eles poderão te ajudar com essa informação!
...
- As aulas do Ensino Fundamental II e do Ensino Medio comecam em 2 de fevereiro de 2026.
- A cerimonia interna de conclusao do Ensino Fundamental II esta prevista para 12 de dezembro de 2026, no fim da tarde, apos o encerramento do ano letivo.
- `llamaindex`: status 200, latency 118.3ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Hoje a base publica de Colegio Horizonte nao traz um marco institucional estruturado para essa data.
- `specialist_supervisor`: status 200, latency 128.0ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:travel_planning`
  answer: Para planejar uma viagem sem atrapalhar a vida escolar, vale observar estes marcos publicos antes de fechar datas:
- O ciclo publico de matricula para 2026 foi aberto em 6 de outubro de 2025, com pre-cadastro, visita orientada opcional e triagem inicial de documentos.
- As aulas do Ensino Fundamental II e do Ensino Medio comecam em 2 de fevereiro de 2026.
- A cerimonia interna de conclusao do Ensino Fundamental II esta prevista para 12 de dezembro de 2026, no fim da tarde, apos o encerramento do ano letivo.
...
- `langgraph`: status 200, latency 6027.3ms, keyword pass `True`, quality `100`, reason `planejamento semantico publico encontrou um ato estruturado mais adequado ao turno`
  answer: Olá! No nosso calendário, os marcos institucionais e eventos coletivos são sempre abertos e divulgados para toda a comunidade. Já os detalhes individuais de cada aluno, convites específicos, protocolos e informações financeiras são acessados de forma segura, com autenticação. Fique tranquila, sempre que houver alguma atualização importante, avisaremos pelos nossos canais oficiais e no portal.
- `crewai`: status 200, latency 20276.6ms, keyword pass `True`, quality `100`, reason `crewai_public_flow_completed`
...
- `python_functions`: status 200, latency 121.3ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
...
- `llamaindex`: status 200, latency 137.5ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: No calendario publico, o que costuma ficar aberto para familias sao marcos institucionais gerais e eventos coletivos. O que depende de autenticacao ou contexto interno sao detalhes individuais por aluno, convites direcionados, protocolos, situacoes financeiras e acompanhamentos protegidos. Sempre que houver ajuste relevante no calendario de avaliacao, a escola atualiza o portal e reforca a comunicacao pelos canais oficiais. O ambiente digital amplia o atendimento, mas nao substitui todos os processos formais.
- `specialist_supervisor`: status 200, latency 117.0ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:restricted_document_denied`
...
- `langgraph`: status 200, latency 6067.7ms, keyword pass `True`, quality `100`, reason `datas institucionais publicas devem vir de leitura estruturada e auditavel`
  answer: Que bom que você se interessa pelo nosso calendário! O ano escolar no Colégio Horizonte se organiza assim: a admissão para 2026 começou em 6 de outubro de 2025, com pré-cadastro e visita. As aulas do Fundamental II e Médio iniciam em 2 de fevereiro de 2026, e a cerimônia de conclusão do Fundamental II será em 12 de dezembro de 2026.
- `crewai`: status 200, latency 20290.3ms, keyword pass `True`, quality `100`, reason `crewai_public_flow_completed`
  answer: Nao consegui concluir essa resposta agora pelo caminho principal configurado. Tente reformular em uma frase mais direta ou repetir em instantes.
- `python_functions`: status 200, latency 138.8ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
...
- `llamaindex`: status 200, latency 122.5ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Hoje a base publica de Colegio Horizonte nao traz um marco institucional estruturado para essa data.
- `specialist_supervisor`: status 200, latency 129.8ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:year_three_phases`
  answer: Admissao: O ciclo publico de matricula para 2026 foi aberto em 6 de outubro de 2025, com pre-cadastro, visita orientada opcional e triagem inicial de documentos.
...
- `langgraph`: status 200, latency 162.0ms, keyword pass `True`, quality `100`, reason `o turno atual trouxe pistas explicitas de outro dominio protegido e o roteamento foi corrigido`
...
- `crewai`: status 200, latency 749.4ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
  answer: Hoje os documentos exigidos para a matricula publicados pela escola sao:
- Formulario cadastral preenchido
- Documento de identificacao do aluno
- CPF do aluno, quando houver
...
- Comprovante de residencia atualizado
- Documento de identificacao do responsavel legal
- `python_functions`: status 200, latency 161.9ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
  answer: Hoje os documentos exigidos para a matricula publicados pela escola sao:
- Formulario cadastral preenchido
- Documento de identificacao do aluno
- CPF do aluno, quando houver
...
- Documento de identificacao do responsavel legal
Se quiser, eu tambem posso explicar as etapas da matricula ou como funciona o envio inicial desses documentos.
- `specialist_supervisor`: status 200, latency 129.1ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:admissions_documents`
...
- `crewai`: status 200, latency 867.6ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
...
- `python_functions`: status 200, latency 178.4ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
  answer: No Colegio Horizonte, a referencia publica de aprovacao e media 7,0/10. Quando a media fica abaixo da referencia, a escola combina monitorias, plantoes e orientacoes de recuperacao conforme o calendario pedagogico. A referencia publica de aprovacao usada nas orientacoes do colegio e 7,0, com fechamento final sempre acompanhado pela equipe pedagogica no periodo letivo.
- `llamaindex`: status 200, latency 151.3ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: No Colegio Horizonte, a referencia publica de aprovacao e media 7,0/10. Quando a media fica abaixo da referencia, a escola combina monitorias, plantoes e orientacoes de recuperacao conforme o calendario pedagogico. A referencia publica de aprovacao usada nas orientacoes do colegio e 7,0, com fechamento final sempre acompanhado pela equipe pedagogica no periodo letivo.
- `specialist_supervisor`: status 200, latency 134.2ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:academic_policy_overview`
...
- `langgraph`: status 200, latency 140.8ms, keyword pass `True`, quality `100`, reason `fato institucional canonico deve vir de fonte estruturada`
...
- `crewai`: status 200, latency 197.8ms, keyword pass `True`, quality `100`, reason `crewai_protected_auth_required`
  answer: Essa consulta depende de autenticacao e vinculo da sua conta no Telegram. Use o portal da escola para gerar o codigo de vinculacao e depois envie o comando /start link_<codigo> ao bot.
- `python_functions`: status 200, latency 139.7ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
  answer: No Colegio Horizonte, a referencia publica minima de frequencia e 75,0% por componente. Se a frequencia de um componente cair abaixo de 75%, o estudante entra em alerta academico e a coordenacao pode acionar a familia. A permanencia abaixo desse limite pode comprometer a aprovacao por frequencia. A escola acompanha justificativas, recorrencia e necessidade de plano de recomposicao junto a familia e ao estudante.
- `llamaindex`: status 200, latency 131.7ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: No Colegio Horizonte, a referencia publica minima de frequencia e 75,0% por componente. Se a frequencia de um componente cair abaixo de 75%, o estudante entra em alerta academico e a coordenacao pode acionar a familia. A permanencia abaixo desse limite pode comprometer a aprovacao por frequencia. A escola acompanha justificativas, recorrencia e necessidade de plano de recomposicao junto a familia e ao estudante.
- `specialist_supervisor`: status 200, latency 121.5ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:conduct_frequency_punctuality`
...
- `langgraph`: status 200, latency 5624.1ms, keyword pass `True`, quality `100`, reason `fato institucional canonico deve vir de fonte estruturada`
...
- `crewai`: status 200, latency 210.6ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
  answer: Sim. Biblioteca Aurora: biblioteca   Biblioteca Aurora   service   True   Atendimento ao publico de segunda a sexta, das 7h30 as 18h00..
- `python_functions`: status 200, latency 150.0ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
  answer: A biblioteca se chama Biblioteca Aurora e funciona de segunda a sexta, das 7h30 as 18h00.
- `llamaindex`: status 200, latency 5232.0ms, keyword pass `True`, quality `100`, reason `llamaindex_public_profile`
  answer: Sim. O Colegio Horizonte tem a Biblioteca Aurora. Atendimento ao publico de segunda a sexta, das 7h30 as 18h00.
- `specialist_supervisor`: status 200, latency 122.6ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_fast_path:library_hours`
  answer: A biblioteca se chama Biblioteca Aurora e funciona Atendimento ao publico de segunda a sexta, das 7h30 as 18h00.
...
- `langgraph`: status 200, latency 135.7ms, keyword pass `True`, quality `100`, reason `fato institucional canonico deve vir de fonte estruturada`
...
- `crewai`: status 200, latency 174.9ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
...
No processo de matricula, o envio inicial pode ser digital. A validacao final continua com a secretaria.
O chat aberto nao deve receber documentos completos ou dados sensiveis. Quando possivel, use o portal autenticado ou o email institucional.
- `llamaindex`: status 200, latency 131.1ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Sim. O envio inicial de documentos pode ser feito por canal digital.
Hoje os canais mais diretos publicados para isso sao:
- portal institucional
- email da secretaria
...
No processo de matricula, o envio inicial pode ser digital. A validacao final continua com a secretaria.
O chat aberto nao deve receber documentos completos ou dados sensiveis. Quando possivel, use o portal autenticado ou o email institucional.
- `specialist_supervisor`: status 200, latency 122.8ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_tool_first:document_submission_policy`
...
- `langgraph`: status 200, latency 160.5ms, keyword pass `True`, quality `100`, reason `fato institucional canonico deve vir de fonte estruturada`
...
- `crewai`: status 200, latency 177.8ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
...
O chat aberto nao deve receber documentos completos ou dados sensiveis. Quando possivel, use o portal autenticado ou o email institucional.
- `llamaindex`: status 200, latency 131.3ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Hoje a familia precisa entender quatro frentes publicas deste fluxo:
- Secretaria: recebe declaracoes, historico, atualizacoes cadastrais e orientacoes administrativas.
- Portal institucional: centraliza protocolo e envio digital inicial de documentos.
- Credenciais: login e senha do portal continuam sendo a base de acesso; se precisar recuperar acesso, o melhor caminho e a secretaria ou o suporte digital.
- Documentos: o envio inicial pode ser feito por portal institucional, email da secretaria ou secretaria presencial.
O chat aberto nao deve receber documentos completos ou dados sensiveis. Quando possivel, use o portal autenticado ou o email institucional.
- `specialist_supervisor`: status 200, latency 120.1ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_fast_path:service_credentials_bundle`
  answer: Hoje a familia precisa entender quatro frentes publicas deste fluxo:
- Secretaria: recebe declaracoes, historico, atualizacoes cadastrais e orientacoes administrativas.
- Portal institucional: centraliza protocolo e envio digital inicial de documentos.
...
- `langgraph`: status 200, latency 150.9ms, keyword pass `True`, quality `100`, reason `fato institucional canonico deve vir de fonte estruturada`
...
- `crewai`: status 200, latency 184.1ms, keyword pass `True`, quality `100`, reason `crewai_public_fast_path`
...
- `python_functions`: status 200, latency 144.2ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
  answer: O edital disciplina pedidos de bolsa social, desconto institucional, politica para irmaos e condicoes de manutencao do beneficio no ano letivo de 2026. Pedidos devem ser feitos dentro da janela informada pela escola, com envio de documentacao socioeconomica e demais comprovantes solicitados no regulamento especifico. A concessao de bolsas e descontos depende de analise documental, criterios internos, disponibilidade orcamentaria e validacao administrativa. Beneficios podem depender de cumprimento das condicoes previstas, como adimplencia, atualizacao cadastral e apresentacao tempestiva de documentos complementares quando solicitados. A rematricula para estudantes ja vinculados ao Colegio Horizonte ocorre em janela administrativa propria, divulgada no portal e reforcada pelos canais oficiais. Pedidos de transferencia para ingresso durante o ano letivo dependem de vaga, analise academica, conferencia documental e alinhamento com a serie pretendida. Quando a familia deseja transferir o aluno para outra instituicao, a solicitacao deve ser formalizada junto a secretaria. Pedidos de cancelamento devem ser apresentados pelo responsavel legal por canal oficial e podem exigir confirmacao adicional da secretaria ou do financeiro. Declaracoes simples costumam ser liberadas em ate dois dias uteis.
- `llamaindex`: status 200, latency 132.6ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: O edital disciplina pedidos de bolsa social, desconto institucional, politica para irmaos e condicoes de manutencao do beneficio no ano letivo de 2026. Pedidos devem ser feitos dentro da janela informada pela escola, com envio de documentacao socioeconomica e demais comprovantes solicitados no regulamento especifico. A concessao de bolsas e descontos depende de analise documental, criterios internos, disponibilidade orcamentaria e validacao administrativa. Beneficios podem depender de cumprimento das condicoes previstas, como adimplencia, atualizacao cadastral e apresentacao tempestiva de documentos complementares quando solicitados. A rematricula para estudantes ja vinculados ao Colegio Horizonte ocorre em janela administrativa propria, divulgada no portal e reforcada pelos canais oficiais. Pedidos de transferencia para ingresso durante o ano letivo dependem de vaga, analise academica, conferencia documental e alinhamento com a serie pretendida. Quando a familia deseja transferir o aluno para outra instituicao, a solicitacao deve ser formalizada junto a secretaria. Pedidos de cancelamento devem ser apresentados pelo responsavel legal por canal oficial e podem exigir confirmacao adicional da secretaria ou do financeiro. Declaracoes simples costumam ser liberadas em ate dois dias uteis.
- `specialist_supervisor`: status 200, latency 125.3ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_fast_path:bolsas_and_processes`
  answer: O edital disciplina pedidos de bolsa social, desconto institucional, politica para irmaos e condicoes de manutencao do beneficio no ano letivo de 2026. Pedidos devem ser feitos dentro da janela informada pela escola, com envio de documentacao socioeconomica e demais comprovantes solicitados no regulamento especifico. A concessao de bolsas e descontos depende de analise documental, criterios internos, disponibilidade orcamentaria e validacao administrativa. Beneficios podem depender de cumprimento das condicoes previstas, como adimplencia, atualizacao cadastral e apresentacao tempestiva de documentos complementares quando solicitados. A rematricula para estudantes ja vinculados ao Colegio Horizonte ocorre em janela administrativa propria, divulgada no portal e reforcada pelos canais oficiais. Pedidos de transferencia para ingresso durante o ano letivo dependem de vaga, analise academica, conferencia documental e alinhamento com a serie pretendida. Quando a familia deseja transferir o aluno para outra instituicao, a solicitacao deve ser formalizada junto a secretaria. Pedidos de cancelamento devem ser apresentados pelo responsavel legal por canal oficial e podem exigir confirmacao adicional da secretaria ou do financeiro. Declaracoes simples costumam ser liberadas em ate dois dias uteis.
...
- `langgraph`: status 200, latency 1901.4ms, keyword pass `True`, quality `100`, reason `planejamento semantico publico encontrou um ato estruturado mais adequado ao turno`
...
- `crewai`: status 200, latency 214.1ms, keyword pass `True`, quality `100`, reason `crewai_protected_auth_required`
  answer: Essa consulta depende de autenticacao e vinculo da sua conta no Telegram. Use o portal da escola para gerar o codigo de vinculacao e depois envie o comando /start link_<codigo> ao bot.
- `python_functions`: status 200, latency 137.1ms, keyword pass `True`, quality `100`, reason `python_functions_native_contextual_public_answer`
...
- `llamaindex`: status 200, latency 123.8ms, keyword pass `True`, quality `100`, reason `contextual_public_direct_answer`
  answer: Atestados medicos e justificativas formais podem ser usados para apoiar analise de ausencia, segunda chamada ou adaptacao temporaria de rotina. Pedidos de segunda chamada exigem justificativa no prazo orientado pela escola.
- `specialist_supervisor`: status 200, latency 123.7ms, keyword pass `True`, quality `100`, reason `specialist_supervisor_fast_path:health_second_call`
  answer: Atestados medicos e justificativas formais podem ser usados para apoiar analise de ausencia, segunda chamada ou adaptacao temporaria de rotina. Pedidos de segunda chamada exigem justificativa no prazo orientado pela escola.
