...
def test_langgraph_semantic_ingress_supports_turn_frame_scope_boundary(monkeypatch) -> None:
    ...
    preview = SimpleNamespace(
        ...
        selected_tools=["get_public_school_profile"],
        graph_path=["langgraph", "turn_frame:scope_boundary"],
    )
    state = {
        "request": SimpleNamespace(message="qual horário da biblioteca pública da cidade?", channel=SimpleNamespace(value="web")),
        "settings": SimpleNamespace(),
        "preview": preview,
        "actor": None,
        "conversation_context": {"recent_messages": []},
        "school_profile": {"school_name": "Colegio Horizonte"},
        "effective_conversation_id": "conv-turn-frame-boundary",
        "engine_name": "langgraph",
        "engine_mode": "dedicated",
        "semantic_ingress_plan": None,
        "turn_frame_public_plan": None,
        "turn_frame": SimpleNamespace(conversation_act="scope_boundary"),
        "langgraph_trace_metadata": {},
    }
...
def test_langgraph_semantic_ingress_blocks_external_city_library_query_even_with_public_plan(
    monkeypatch,
) -> None:
    async def fake_persist_trace(**_kwargs):
        return None
    ...
    state = {
        ...
        "conversation_context": {"recent_messages": []},
        "school_profile": {"school_name": "Colegio Horizonte"},
        "effective_conversation_id": "conv-turn-frame-external-library",
        "engine_name": "langgraph",
        "engine_mode": "dedicated",
        "semantic_ingress_plan": None,
        "turn_frame_public_plan": SimpleNamespace(capability="public.facilities.library.hours"),
