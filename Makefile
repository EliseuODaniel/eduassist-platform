...
LOCAL_LLM_QWEN_PROFILE := --profile local-llm-qwen3-4b
...
compose-up-dedicated-core-qwen3-4b-local: env
	LLM_MODEL_PROFILE=qwen3_4b_instruct_local OPENAI_API_MODE=chat_completions docker compose $(SPECIALIST_PROFILE) $(LOCAL_LLM_QWEN_PROFILE) --env-file $(ENV_FILE) -f $(COMPOSE_FILE) up -d --build $(DEDICATED_CORE_SERVICES_BASE) local-llm-qwen3-4b
...
local-llm-qwen3-4b-down: env
	docker compose $(LOCAL_LLM_QWEN_PROFILE) --env-file $(ENV_FILE) -f $(COMPOSE_FILE) stop local-llm-qwen3-4b

local-llm-qwen3-4b-logs: env
	docker compose $(LOCAL_LLM_QWEN_PROFILE) --env-file $(ENV_FILE) -f $(COMPOSE_FILE) logs -f --tail=200 local-llm-qwen3-4b
