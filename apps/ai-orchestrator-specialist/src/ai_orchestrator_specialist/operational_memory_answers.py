...
async def maybe_operational_memory_follow_up_answer(
    ...
    *,
    ...
    if deps.looks_like_public_doc_bundle_request(ctx.request.message):
        return None
    student_hint = deps.is_student_name_only_followup(ctx.actor, ctx.request.message)
    multi_domains = deps.effective_multi_intent_domains(memory, ctx.request.message)
    subject_hint = deps.subject_hint_from_text(ctx.request.message) or memory.active_subject

    if len(multi_domains) >= 2 and "academic" in multi_domains and "finance" in multi_domains:
        target_name = student_hint or deps.student_hint_from_message(ctx.actor, ctx.request.message) or memory.active_student_name
        if target_name:
    ...
    if (
        ...
        and (subject_hint or deps.looks_like_subject_followup(ctx.request.message))
    ):
